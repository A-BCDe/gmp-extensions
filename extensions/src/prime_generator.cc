...
namespace gmp_extensions {
       ...
       sieve(sieve_size) {
        for(size_t pivot = 0; pivot < bound; pivot += sieve_size) {
            ...
            for(auto const &p: primes) {
                ...
                for(auto i = idxui; i < sieve_size; i += pui) { sieve[i] = true; }
            }
            ...
            for(size_t i = 0; i < sieve_size && i < bound - pivot; i++) {
                ...
                for(size_t j = i + cur; j < sieve_size; j += cur) { sieve[j] = true; }
            }
        }
    }
    ...
    void prime_generator::print_primes(std::ostream &os) const {
        ...
        for(auto const &p: primes) { os << p.get_str() << ' '; }
    }
     ...
     * @param f Some function which gets a prime as an input and returns a boolean
     * value.
