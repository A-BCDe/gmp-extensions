...
namespace gmp_extensions {
     ...
     */
    inline static integer_polynomial make_monomial(mpz_class const &coef, mpz_class deg, integer_polynomial const &poly, mpz_class const &p) {
        assert(!poly.is_zero());
        assert(is_prime(p));
        assert(coef % p != 0);
     ...
     * @param coef A vector of integers which denotes the coefficients of the
     * polynomial
     */
    ...
    integer_polynomial::integer_polynomial(std::vector<std::string> const &coef) {
        ...
        for(std::string const &now: coef) { this->coef.emplace_back(now.c_str()); }
        clean();
    }
    ...
    integer_polynomial::integer_polynomial(vector const &v) {
        ...
        for(size_t i = 0; i < v.length(); i++) { coef.emplace_back(v[i]); }
        clean();
    }
    ...
    integer_polynomial integer_polynomial::monomial(mpz_class const &coefficient, size_t deg) {
        ...
        for(size_t i = 0; i < deg; i++) { polynomial.coef.emplace_back(0); }
     ...
     * @param p A prime
     * @return true if the polynomial is divisible by poly in modulo p, false
     * otherwise
     */
    bool integer_polynomial::is_divisible_modulo(integer_polynomial const &poly, mpz_class const &p) const {
        ...
        for(auto &now: monic_poly.coef) { now %= p; }
    ...
    bool integer_polynomial::is_divisible(integer_polynomial const &poly) const {
        ...
        if(!polynomial_p) { return false; }
        delete[] polynomial_p;
        return true;
    }

    /**
     * Calculates the polynomial modulo poly on Z_p, then saves to and returns
     * *this.
     * @param poly The divisor
    ...
    integer_polynomial &integer_polynomial::modulo_eq(integer_polynomial poly, mpz_class const &p) {
        ...
        if(!poly.is_monic()) {
            ...
            for(size_t i = 0; i < poly.coef.size() - 1; i++) { poly.coef[i] %= p; }
        }
        ...
        for(auto &now: coef) { now = ((now % p) + p) % p; }
        return *this;
    }
    ...
    /**
     * Makes the polynomial primitive by dividing proper integer to every
     * coefficients.
    ...
    integer_polynomial &integer_polynomial::primitive_eq() {
        ...
        mpz_class g = coef[0];
        mpz_class tmp;
        for(size_t i = 1; i < coef.size(); i++) { g = gcd(g, coef[i]); }
        for(auto &now: coef) {
    ...
    /**
     * Calculates the polynomial primitive by dividing proper integer to every
     * coefficients.
    ...
    integer_polynomial &integer_polynomial::derivative_eq() {
        for(size_t i = 0; i < coef.size() - 1; i++) { coef[i] = coef[i + 1] * (i + 1); }
        coef.pop_back();
        clean();
        return *this;
    }
    ...
    mpz_class integer_polynomial::discriminant() const {
        ...
        if(degree() % 4 != 0 && (degree() - 1) % 4 != 0) { res = -tmp; }
        else { res = tmp; }
        return res;
    }
     ...
     * Returns a nullptr if the memory allocation failed, or the division is not
     * exact.
     * @param poly The divisor
     * @return A pointer to the quotient if the polynomial is divided exactly by
     * poly and is allocated correctly, nullptr otherwise
     */
    integer_polynomial *integer_polynomial::divexact(integer_polynomial const &poly) const {
        assert(!poly.is_zero());
        if(is_zero()) { return new(std::nothrow) integer_polynomial; }
        if(coef.size() < poly.coef.size()) { return nullptr; }
        integer_polynomial polynomial(*this);
    ...
    /**
     * Dividing the polynomial provided that poly divides the polynomial exactly in
     * Z_p.
     * @param poly The divisor
     * @param p A prime
     ...
     * @return A vector of irreducible polynomials which multiplies up to the
     * polynomial
     */
    ...
    std::vector<integer_polynomial> integer_polynomial::factorize_squarefree() const {
        ...
        for(auto const &now: coef) { f += now * now; }
        ...
        mpz_class p(pg.smallest_prime([this](mpz_class const &p) {
            return !mpz_divisible_p(leading().get_mpz_t(), p.get_mpz_t()) && is_square_free(p);
        }));
        mpz_class p_power_l;
        ...
        std::sort(hensel_v.begin(), hensel_v.end(), [](std::pair<integer_polynomial, size_t> const &a, std::pair<integer_polynomial, size_t> const &b) {
...
LP:
        while(!T.empty()) {
            ...
            for(auto now_idx: T) {  // Dynamic Programming
                ...
                for(auto prev: degree_set) { degree_set_tmp.insert(prev + now_deg); }
            ...
            for(size_t now_deg: degree_set) {
                ...
                for(size_t i = 0; i < k; i++) {
                    vector v(j);
                    for(size_t ii = 0; ii < u.coef.size(); ii++) { v[ii + i] = u[u.degree() - ii]; }
     ...
     * @param p A prime
     * @return A vector of irreducible polynomials modulo p which multiplies up to
     * the polynomial modulo p
     */
    std::vector<integer_polynomial> integer_polynomial::factorize_squarefree_modulo(mpz_class const &p) const {
        ...
        for(auto &v: space) {
            for(size_t i = 0; i < degree(); i++) { v[i] = ((v[i] % p) + p) % p; }
        }
...
END:
     ...
     */
    std::pair<integer_polynomial, integer_polynomial> integer_polynomial::hensel_lifting(integer_polynomial g, integer_polynomial h, mpz_class const &p, size_t k, size_t desired_power) const {
        mpz_class tmp;
        ...
        while(current_power <= desired_power) {
            ...
            for(size_t i = 0; i < g.degree(); i++) {
                for(size_t j = 0; j <= h.degree(); j++) { matrix(i + j, i) = h[j]; }
            }
            for(size_t i = 0; i <= h.degree(); i++) {
                for(size_t j = 0; j <= g.degree(); j++) { matrix(i + j, i + g.degree()) = g[j]; }
            }
            ...
            g += g_star * current_modulo;
            h += h_star * current_modulo;
    ...
    integer_polynomial &integer_polynomial::mul_eq(integer_polynomial const &poly) {
        ...
        for(size_t i = 0; i < coef.size(); i++) {
            for(size_t j = 0; j < poly.coef.size(); j++) { tmp[i + j] += coef[i] * poly.coef[j]; }
        }
        coef = std::move(tmp);
        clean();
        return *this;
    ...
    integer_polynomial gcd(integer_polynomial a, integer_polynomial b) {
        ...
        if(b.coef.empty()) {
            ...
            mpz_class q;
            for(size_t i = 1; i < a.coef.size(); i++) { g = gcd(g, a.coef[i]); }
            for(auto &now: a.coef) {
     ...
     */
    integer_polynomial gcd(integer_polynomial a, integer_polynomial b, mpz_class const &p) {
        assert(is_prime(p));
        ...
        if(b.is_zero()) {
            ...
            // and modulo p
            for(auto &now: a.coef) { now = ((now % p) + p) % p; }

            // multiply inverse of leading coefficient
            // and modulo p
            mpz_class inv;
            invert(inv, a.leading(), p);
            for(size_t i = 0; i < a.coef.size() - 1; i++) { a.coef[i] = (a.coef[i] * inv) % p; }
            a.coef.back() = 1;
            return a;
        }
