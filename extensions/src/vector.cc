...
namespace gmp_extensions {
    ...
    vector::vector(std::vector<mpz_class> vec)
     : vec(std::move(vec)) {}
    ...
    bool vector::is_zero() const {
        return std::all_of(vec.begin(), vec.end(), [](mpz_class const &n) { return n == 0; });
    }

    mpz_class vector::norm2() const {
        mpz_class res = 0;
        for(auto const &now: vec) { res += now * now; }
        return res;
    }
    ...
    vector &vector::neg_eq() {
        for(auto &now: vec) { now = -now; }
        return *this;
    }

    vector &vector::add_eq(vector const &v) {
        assert(length() == v.length());
        for(size_t i = 0; i < length(); i++) { vec[i] += v[i]; }
        return *this;
    }
    ...
    mpz_class inner_product(vector const &v1, vector const &v2) {
        ...
        mpz_class res(0);
        for(size_t i = 0; i < v1.length(); i++) { res += v1[i] * v2[i]; }
        return res;
    }
    ...
    std::ostream &operator<<(std::ostream &os, vector const &v) {
        ...
        os << v[0].get_str();
        for(size_t i = 1; i < v.length(); i++) { os << ' ' << v[i].get_str(); }
        return os;
    }
