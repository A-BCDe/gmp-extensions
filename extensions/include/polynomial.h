...
namespace gmp_extensions {
    ...
    public:
        ...
        [[nodiscard]]
        static integer_polynomial monomial(mpz_class const &, size_t);
        [[nodiscard]]
        static integer_polynomial constant(mpz_class const &);
        ...
        [[nodiscard]]
        bool is_square_free(mpz_class const &p) const;

        [[nodiscard]]
        bool is_monic() const noexcept {
            return !coef.empty() && coef.back() == 1;
        }

        [[nodiscard]]
        bool is_divisible_modulo(integer_polynomial const &, mpz_class const &) const;
        ...
        [[nodiscard]]
        integer_polynomial modulo(integer_polynomial, mpz_class const &) const;

        integer_polynomial &primitive_eq();
        [[nodiscard]]
        integer_polynomial primitive() const;

        integer_polynomial &derivative_eq();
        [[nodiscard]]
        integer_polynomial derivative() const;

        [[nodiscard]]
        mpz_class discriminant() const;

        [[nodiscard]]
        integer_polynomial *divexact(integer_polynomial const &) const;

        integer_polynomial &divexact_modulo_eq(integer_polynomial const &, mpz_class const &);
        [[nodiscard]]
        integer_polynomial divexact_modulo(integer_polynomial const &poly, mpz_class const &p) const;
        ...
        mpz_class &operator[](size_t idx) {
            assert(idx < coef.size());
            return coef[idx];
        }

        mpz_class const &operator[](size_t idx) const {
            assert(idx < coef.size());
            return coef[idx];
        }
        ...
        [[nodiscard]]
        integer_polynomial
        operator+(integer_polynomial const &poly) const {
            return add(poly);
        }
        ...
        [[nodiscard]]
        integer_polynomial
        operator*(mpz_class const &n) const {
            return mul_scalar(n);
        }

        [[nodiscard]]
        friend integer_polynomial
        operator*(mpz_class const &n, integer_polynomial const &poly) {
            return poly * n;
        }
    ...
    private:
        ...
        [[nodiscard]]
        integer_polynomial neg() const;
        [[nodiscard]]
        integer_polynomial add(integer_polynomial const &) const;
        [[nodiscard]]
        integer_polynomial sub(integer_polynomial const &) const;
        [[nodiscard]]
        integer_polynomial mul(integer_polynomial const &) const;
        [[nodiscard]]
        integer_polynomial mul_scalar(mpz_class const &) const;

        [[nodiscard]]
        bool equal(integer_polynomial const &) const;
    };
